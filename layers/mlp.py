...
class MLP:
    ...
    def __init__(self, dkey,n_embed, seq_len, batch_size, eta, optim_type, wub , wlb, prefix, tau_m, **kwargs):
        ...
        self.W_mlp1 = HebbianSynapse(f"{prefix}W_mlp1", shape=(n_embed, 4*n_embed), batch_size = batch_size * seq_len, eta=eta, weight_init=dist.uniform(amin=wlb, amax=wub),
                    bias_init=dist.constant(value=0.), w_bound=0., optim_type=optim_type, sign_value=-1.0, key=subkeys[4],prior=("l1l2", (0.001, 0.001)),pre_wght=config.hebb_scale, post_wght=config.hebb_scale)
        self.W_mlp2 = HebbianSynapse(
                    ...
                    bias_init=dist.constant(value=0.), w_bound=0., optim_type=optim_type, sign_value=-1.0, key=subkeys[5],prior=("l1l2", (0.001, 0.001)),pre_wght=config.hebb_scale, post_wght=config.hebb_scale)
