...
class MLP:
    ...
    def __init__(self, dkey,n_embed, seq_len, batch_size, eta, optim_type, wub , wlb, prefix, tau_m, w_bound=0.5, **kwargs):
        dkey, *subkeys = random.split(dkey, 10)
        pre_scale = 1.0 / math.sqrt(max(batch_size * seq_len, 1))
        hebb_prior = getattr(config, "hebb_prior", (0.001, 0.1))
       

        self.z_mlp1 = RateCell(f"{prefix}z_mlp1", n_units=n_embed, tau_m=tau_m, act_fx="identity",
                       batch_size=batch_size * seq_len, prior=("gaussian", 0.1), resist_scale=0.5)
        self.z_mlp2 = RateCell(f"{prefix}z_mlp2", n_units= 4* n_embed, tau_m= tau_m, act_fx="gelu",
                       batch_size=batch_size * seq_len, prior=("gaussian", 0.1), resist_scale=0.5)
        ...
        self.W_mlp2 = HebbianSynapse(
                    ...
                    bias_init=dist.constant(value=0.), w_bound=w_bound, optim_type=optim_type, sign_value=-1.0, key=subkeys[5],
                    prior=("l1l2", hebb_prior), pre_wght=pre_scale)
