...
class MLP:
    ...
    def __init__(self, dkey,n_embed, seq_len, batch_size, eta, optim_type, wub , wlb, prefix, tau_m, **kwargs):
        ...
        self.z_mlp1 = ClippedRateCell(f"{prefix}z_mlp1", n_units=n_embed, tau_m=tau_m, act_fx="identity", batch_size=batch_size * seq_len, prior=("gaussian", 0.2))
        self.z_mlp2 = ClippedRateCell(f"{prefix}z_mlp2", n_units= 4* n_embed, tau_m= tau_m, act_fx="gelu", batch_size=batch_size * seq_len, prior=("gaussian", 0.2))
        ...
        self.W_mlp1 = HebbianSynapse(f"{prefix}W_mlp1", shape=(n_embed, 4*n_embed), batch_size = batch_size * seq_len, eta=eta, weight_init=dist.gaussian(mean=0.0, std=0.02),
                    bias_init=dist.constant(value=0.), w_bound=0.5, optim_type=optim_type, sign_value=-1.0, key=subkeys[4],prior=("l1l2", (0.02, 0.02)))
        self.W_mlp2 = HebbianSynapse(
                    ...
                    bias_init=dist.constant(value=0.), w_bound=0.5, optim_type=optim_type, sign_value=-1.0, key=subkeys[5],prior=("l1l2", (0.02, 0.02)))
