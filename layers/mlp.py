...
class MLP:
    ...
    def __init__(self, dkey,n_embed, seq_len, batch_size, eta, optim_type, wub , wlb, prefix, tau_m, **kwargs):
        ...
        self.W_mlp1 = HebbianSynapse(f"{prefix}W_mlp1", shape=(n_embed, 4*n_embed), batch_size = batch_size * seq_len, eta=eta, weight_init=dist.uniform(low=wlb, high=wub),
                bias_init=dist.constant(value=0.), w_bound=0., optim_type=optim_type, sign_value=-1.0, key=subkeys[4],prior=("constant", 0.))
        self.W_mlp2 = HebbianSynapse(
                f"{prefix}W_mlp2", shape=(4*n_embed, n_embed), batch_size= batch_size * seq_len, eta=eta, weight_init=dist.uniform(low=wlb, high=wub),
                bias_init=dist.constant(value=0.), w_bound=0., optim_type=optim_type, sign_value=-1.0, key=subkeys[5],prior=("constant", 0.))
        ...
        self.E_mlp1 = StaticSynapse(f"{prefix}E_mlp1", shape=(4 * n_embed,n_embed), weight_init=dist.uniform(low=wlb, high=wub), bias_init=None, key=subkeys[4])
        self.E_mlp = StaticSynapse(f"{prefix}E_mlp", shape=(n_embed, 4 * n_embed), weight_init=dist.uniform(low=wlb, high=wub), bias_init=None, key=subkeys[4])
