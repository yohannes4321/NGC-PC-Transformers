...
class RateCell(JaxComponent): ## Rate-coded/real-valued cell
    ...
    def __init__(
self, name, n_units, tau_m, prior=("gaussian", 0.), act_fx="identity", 
            output_scale=1., threshold=("none", 0.), integration_type="euler", 
        ...
        if output_scale == "auto":
            # Logical Variance Scaling: 1/sqrt(n_units)
            self.output_scale = 1.0 / jnp.sqrt(n_units)
        else:
