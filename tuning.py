...
CASE = int(getattr(base_config, "case", 1))
...
def run_single_trial_efe(trial):
    try:
        ...
        total_train_ce = 0.0  # track CE so we can display CE/PPL per batch
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            try:
                ...
                # Always compute CE/PPL for logging (regardless of CASE)
            ...
            if CASE == 2:
                trial.report((current_efe * 0.5 + avg_train_ce * 0.5), batch_idx)
            else:
            ...
            if batch_idx % 2 == 0:
                ...
                print(
                    ...
                    f"PPL={float(batch_train_ppl):.4f} | Avg EFE={current_efe:.4f} | "
                    f"Avg CE={avg_train_ce:.4f} | Time={elapsed:.1f}s"
                )
...
def case2_efe_to_ce_complete():
    Path("tuning").mkdir(exist_ok=True)
    Path("tuning_results").mkdir(exist_ok=True)

    study_efe = optuna.create_study(
        study_name="case1_complete_phase1_efe",
        storage="sqlite:///tuning/case2_complete.db",
        load_if_exists=True,
        direction="minimize",
        sampler=optuna.samplers.TPESampler(seed=42, n_startup_trials=2),
        pruner=optuna.pruners.HyperbandPruner(min_resource=10, max_resource=15, reduction_factor=2)
    ...
    if study_efe.best_trial:
        ...
        print(f"\n{'='*60}")
        print("case 2  EFE AND CE combined COMPLETE")
        print(f"{'='*60}")
        print(f"Best EFE : {best_efe:.4f}")
        print(f"best CE: {best_efe_ce}")
...
def main():
    try:
        if CASE == 1:
            ...
            print("PC TRANSFORMER case 2 combined CE AND EFE HYPERPARAMETER TUNING")
            print("="*60)
            print("="*60)
            results = case2_efe_to_ce_complete()
            if results:
                print(f"\n{'='*60}")
                print("TUNING COMPLETED SUCCESSFULLY")
                print(f"{'='*60}")
            ...
            print("Invalid CASE selection. Set env CASE=1 or CASE=2.")
