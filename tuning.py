...
def parallel_func_phase1(**x):
    ...
    try:
        ...
        return 1e9 # Return huge error on crash

    # FAIL-FAST CHECK: Explosion
    if val > 10000:
        print(f"--- [Worker {worker_id}] EXPLOSION DETECTED (Loss: {val:.1f}). STOPPING. ---", flush=True)
        return 1e9 # Treat as failure

    print(f"--- [Worker {worker_id}] Success. EFE Loss: {val:.4f} ---", flush=True)
    return val
...
def run_smart_phase(optimizer, objective_name, fixed_params=None, max_no_change=3, budget_limit=4):
    ...
    2. Stops early if no improvement is seen after `max_no_change` (e.g., 3 times).
    """
    best_loss = float("inf")
    best_params = None
    no_change_counter = 0
    
    print(f"\n[{objective_name.upper()}] Starting Smart Loop (Max {budget_limit} trials)...")
...
def run_advanced(p1_budget=20, p2_budget=20, num_workers=4):
    print("\n" + "="*60)
    print("STARTING OPTIMIZATION RELAY RACE")
    print("Plan: CMA (EFE) -> Chaining (EFE) -> Portfolio (CE) -> DE (CE)")
    print("="*60)

    # --- STEP 1: CMA (The EFE Foundation) ---
    print(f"\n[STEP 1/4] CMA-ES: FINDING EFE BASELINE")
    ...
    # INFER BEST LOSS (Hack since we don't have it explicitly from rec_cma)
    print("... verifying baseline EFE ...")
    baseline_efe = parallel_func_phase1(**current_best_params)
    print(f">>> STEP 1 FINISHED. Best EFE so far: {baseline_efe:.4f}")

    if baseline_efe > 10000:
        print("!!! FAILURE: EFE is > 10,000. System unstable. Exiting.")
        return

    # Lock the Skeleton for the next steps
    ...
    # User Requirement: "LET CHAIN USE THE BEST VALUE THE CMA GIVE AND MORE OPTIMIZE IT"
    print(f"\n[STEP 2/4] CHAINING: IMPROVING EFE (Max 4 Trials)")
    
    # We use phase2_space which centers around the CMA winner
    ...
    if loss_efe < baseline_efe:
        print(f">>> SUCCESS: Chaining improved EFE from {baseline_efe:.4f} to {loss_efe:.4f}")
        current_best_params = params_efe
    else:
        print(f">>> NOTE: Chaining did not improve EFE. Keeping CMA result.")

    # --- STEP 3: PORTFOLIO (CE Optimization) ---
    # User Requirement: "GIVE ONE FUNCTION MORE ON CE OPTIMIZATION"
    print(f"\n[STEP 3/4] PORTFOLIO: LOWERING CE (ACCURACY) (Max 4 Trials)")
    ...
    if loss_ce < 1000: # Simple sanity check
        current_best_params = params_ce

    # --- STEP 4: DE (Final CE Polish) ---
    print(f"\n[STEP 4/4] DE: FINAL CE POLISH (Max 4 Trials)")
    ...
    print(f"Final Architecture: {fixed_arch}")
    print("="*60)
